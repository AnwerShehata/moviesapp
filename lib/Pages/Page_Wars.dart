...
import '../Details/Details_Wars.dart';

class Page_Wars extends StatefulWidget {
  _BirdState createState() => new _BirdState();
}

class _BirdState extends State<Page_Wars> {
  @override
...
//          color: Colors.white,
          child: new StaggeredGridView.countBuilder(
            //هنا يحدد طول الصورة التي تظهر في العمود اليمين واليسار
              ...
              crossAxisCount: 3,
              itemCount: dataMoviWars.length,                         //هنا المكان الذي يجلب منه البيانات
            ...
            return buildGestureDetector(
                index ,imgUrl: "${dataMoviWars[index].imagUrl}" ,
                onTap: () {Navigator.push(context, MaterialPageRoute(builder: (context)=> Details_Wars(
                imagUrl:"${dataMoviWars[index].imagUrl}",
                name: "${dataMoviWars[index].name}",
                description: "${dataMoviWars[index].description}",
                videoUrl: "${dataMoviWars[index].videoUrl}",
                ) ));});
          }
      ),
          ),
