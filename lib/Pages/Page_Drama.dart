...
class Page_Drama extends StatefulWidget {
  _BirdState createState() => new _BirdState();
}

class _BirdState extends State<Page_Drama> {
  @override
...
//          color: Colors.white,
          child: new StaggeredGridView.countBuilder(
            //هنا يحدد طول الصورة التي تظهر في العمود اليمين واليسار
              ...
              crossAxisCount: 3,
              itemCount: dataMoviesDrama.length,                         //هنا المكان الذي يجلب منه البيانات
            ...
            return buildGestureDetector(
                index ,imgUrl: "${dataMoviesDrama[index].imagUrl}" ,
                onTap: () {Navigator.push(context, MaterialPageRoute(builder: (context)=> Details_Drama(
                imagUrl:"${dataMoviesDrama[index].imagUrl}",
                name: "${dataMoviesDrama[index].name}",
                description: "${dataMoviesDrama[index].description}",
                videoUrl: "${dataMoviesDrama[index].videoUrl}",
                ) ));});
          }
      ),
          ),
